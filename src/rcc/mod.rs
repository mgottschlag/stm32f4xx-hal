...
impl CFGR {
    ...
    /// Declares that the selected frequency is available at the I2S clock input pin.
    ...
    ///
    /// The selected frequency implies an identical PLLI2SR frequency, unless `i2s_ckin()` has
    /// been called with an identical frequency. If PLLI2SR is set to a different frequency, the
    /// clock configuration code may panic if no suitable configuration is found.
    ///
    /// On STM32F410, the PLLR clock output is used instead of PLLI2SR.
    #[cfg(any(
        feature = "stm32f401",
        feature = "stm32f405",
        feature = "stm32f407",
    ...
    /// Selects an I2S clock frequency and enables the I2S clock.
    ///
    /// The same restrictions apply as for `i2s_apb1_clk()`.
    #[cfg(any(
        feature = "stm32f412",
        feature = "stm32f413",
        feature = "stm32f423",
    ...
    ///
    /// If an external I2S clock is provided and the frequency is identical, the external I2S clock
    /// is used for SAI. Otherwise, the PLLSAI "Q" output is used. Only on the STM32F413/423, PLL2
    /// "R" is used instead and can be configured via `plli2s_r()`.
    ///
    /// If a PLL output frequency is specified, the MCU needs to support the ratio between the
    /// frequencies. If not, the clock configuration code will panic.
    #[cfg(any(
        feature = "stm32f413",
        feature = "stm32f423",
        feature = "stm32f427",
    ...
    ///
    /// See `saia_clk()` for information about clock sources.
    #[cfg(any(
        feature = "stm32f413",
        feature = "stm32f423",
        feature = "stm32f427",
    ...
    ///
    /// If an external I2S clock is provided and the frequency is identical, the external I2S clock
    /// is used for SAI. Otherwise, the PLLSAI "Q" output is used.
    ///
    /// If a PLL output frequency is specified, the MCU needs to support the ratio between the
    /// frequencies. If not, the clock configuration code will panic.
    #[cfg(any(feature = "stm32f446",))]
    ...
    ///
    /// TODO: Document restrictions w.r.t. PLL usage.
    #[cfg(any(feature = "stm32f446",))]
    ...
    fn pll_setup(&self, pllsrcclk: u32, pllsysclk: Option<u32>) -> PllSetup {
        ...
        let main_pll = if let Some(i2s_clk) = i2s_clocks.pll_i2s_clk {
            ...
            // so we need an expensive full PLL optimization step.
        ...
        let real_i2s_clocks = i2s_clocks.real(main_pll.plli2sclk, self.i2s_ckin);
        PllSetup {
    ...
    fn pll_setup(&self, pllsrcclk: u32, pllsysclk: Option<u32>) -> PllSetup {
        ...
        let (i2s_pll, real_sai_clk) = if let Some(i2s_clk) = i2s_clocks.pll_i2s_clk {
            ...
            // NOTE: STM32F413/423 have a different divider range!
        ...
        let real_i2s_clocks = i2s_clocks.real(i2s_pll.plli2sclk, self.i2s_ckin);
        let real_sai_clocks = sai_clocks.real(real_sai_clk, self.i2s_ckin);
    ...
    fn pll_setup(&self, pllsrcclk: u32, pllsysclk: Option<u32>) -> PllSetup {
        ...
        let real_i2s_clocks = i2s_clocks.real(i2s_pll.plli2sclk, self.i2s_ckin);
        #[cfg(feature = "stm32f446")]
        let real_sai_clocks = sai_clocks.real(sai_pll.pllsaiclk, self.i2s_ckin);
